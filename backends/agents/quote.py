...
async def load_llm():
 ...
 # hardcoding the access token and model name
    ...
    model_config = transformers.AutoConfig.from_pretrained(model_nm, 
...
async def generate_quote(prompt):
    ...
    sequences = query_pipeline(
        ...
        max_length=10000, # Set the maximum length for the generated sequence
        truncation=True 
    )
...
async def start_up_function(ctx: Context):
    ...
    ctx.logger.info("Agent Quote started successfully and loaded the llm")
